...
const skillBadges = [
  { group: "Frontend", items: ["React.js", "HTML", "CSS", "MERN Stack", "Responsive UI", "REST integration"] },
  { group: "Developer Tools", items: ["Git", "GitHub", "VS Code", "Postman", "Delhivery API", "Meta API", "Apollo", "OOP", "Data Structures"] },
];

function BarGroup({ group, delay }: { group: typeof skillsWithBars[0]; delay: number }) {
  ...
  return (
    ...
    >
      <p
        className="font-mono text-[0.65rem] tracking-[0.12em] uppercase mb-5 pb-3 border-b"
          ...
          <div key={item.name}>
            ...
            >
              <motion.div
                ...
                style={{
                  background: "linear-gradient(90deg, var(--gold), rgba(232,184,75,0.4))",
                }}
                initial={{ width: 0 }}
    ...
    >
      <div ref={headerRef}>
        ...
        <motion.p
          ...
          className="text-[1rem] leading-[1.75] max-w-[520px] mb-12"
      ...
      >
        {skillsWithBars.map((g, i) => (
          <BarGroup key={g.group} group={g} delay={i * 0.1} />
        ))}
      </div>
      ...
      {skillBadges.map((g, i) => {
        ...
        return (
          ...
          >
            <p
              className="font-mono text-[0.65rem] tracking-[0.12em] uppercase mb-4 pb-3 border-b"
              style={{ color: "var(--gold)", borderColor: "var(--border)" }}
            >
              {g.group}
            </p>
